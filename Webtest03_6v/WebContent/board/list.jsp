...
<body bgcolor="<%=bodyback_c%>">
 ...
 %><image src="images/hot.gif" border="0" height="16"> <%
		...
		<p>
			...
			%>
			<a
				href="list.jsp?pageNum=<%=startPage - 5%>&search=<%=search%>&searchn=<%=searchn%>">[이전]</a>
			<%
			}
			%>
			<%
			...
			%>
			<a
				href="list.jsp?pageNum=<%=startPage + 5%>&search=<%=search%>&searchn=<%=searchn%>">[다음]</a>
			<%
			}
			%>
			<%
